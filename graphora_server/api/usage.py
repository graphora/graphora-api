...
from graphora_server.schemas.usage import (
    UsageReport,
    LimitCheckResult,
    ModelProviderSchema,
    ModelPricingSchema,
...
async def get_usage_summary(
    ...
    try:
        ...
        report = await usage_tracking_service.get_usage_report(
            user_id=user_id, period_start=month_start, period_end=now
        )
        ...
        summary = {
            ...
            "performance": {
                ...
                "success_rate": (
                    str(report.success_rate) if report.success_rate else None
                ),
            },
        }
...
async def get_model_usage_breakdown(
    ...
    try:
        ...
        for model_key, usage in report.model_usage.items():
            provider, model = (
                model_key.split(":", 1) if ":" in model_key else (model_key, "unknown")
            )
            ...
            formatted_models[provider][model] = {
                ...
                "avg_tokens_per_call": (
                    round(usage["total_tokens"] / usage["calls"], 2)
                    if usage["calls"] > 0
                    else 0
                ),
            }
...
@router.get(
    "/usage/pricing/providers/{provider_name}/models",
    response_model=List[ModelPricingSchema],
)
async def get_provider_pricing(provider_name: str) -> List[ModelPricingSchema]:
    ...
    """
    try:
        pricing = await usage_tracking_service.get_model_pricing_by_provider(
            provider_name
        )
        return pricing

...
from graphora_server.services.chunking.chunker import DocumentChunker
...
async def test_chunking(request: ChunkingTestRequest) -> ChunkingTestResponse:
    ...
    try:
        ...
        from graphora_server.services.chunking.hybrid_chunker import DocumentClassifier
        ...
        return ChunkingTestResponse(
            ...
            strategy_used=(
                request.strategy_override.value
                if request.strategy_override
                else config.strategy.value
            ),
    ...
    try:
        ...
        return {
            ...
            "estimated_performance": {
                ...
                "memory_usage": (
                    "low" if config.strategy != ChunkingStrategy.SEMANTIC else "medium"
                ),
                "quality": (
                    "high" if config.strategy == ChunkingStrategy.HYBRID else "medium"
                ),
            },
        }
...
async def analyze_document_for_chunking(
    ...
    try:
        ...
        recommended_config = DEFAULT_CONFIGS.get(
            doc_type.value, DEFAULT_CONFIGS["hybrid"]
        )
        ...
        confidence_indicators = {
            "structured": sum(
                [
                    ...
                    * 0.4,
                    (
                        len([line for line in lines if line.strip()]) / len(lines)
                        if lines
                        else 0
                    )
                    * 0.3,
        ...
        alternative_configs = {
            strategy: config
            for strategy, config in DEFAULT_CONFIGS.items()
        ...
        chunking_estimate = {
            "estimated_chunks": max(
                1, len(request.text) // recommended_config.max_chunk_size
            ),

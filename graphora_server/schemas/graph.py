...
class Node(BaseModel):
    ...
    labels: Optional[List[str]] = Field(
        default=None, description="Additional labels for the node"
    )
...
class GraphResponse(BaseModel):
    ...
    total_nodes: Optional[int] = Field(
        default=0, description="Total number of nodes with this label"
    )
    total_edges: Optional[int] = Field(
        default=0, description="Total number of relationships for these nodes"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Metadata about the graph"
    )

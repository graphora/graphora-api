...
async def quality_validation_task(
    ...
    try:
        ...
        await quality_service.store_quality_results(
            transform_id, quality_results, user_id
        )
        ...
        logger.error(
            f"Quality validation task failed for transform {transform_id}: {e}"
        )
        raise
...
async def auto_approval_check_task(
    ...
    try:
        ...
        if eligibility["eligible"]:
            ...
            logger.info(
                f"Transform {transform_id} requires manual review: {', '.join(eligibility['reasons'])}"
            )
...
async def quality_metrics_calculation_task(
    quality_results: QualityResults, transform_id: str
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Calculating detailed quality metrics for transform {transform_id}"
        )
        ...
        detailed_metrics = {
            ...
            "confidence_analysis": {
                ...
                "low_confidence_entities": [
                    entity_type
                    for entity_type, confidence in metrics.confidence_scores_by_type.items()
                    if confidence < 0.7
                ],
            },
        ...
        logger.error(
            f"Quality metrics calculation failed for transform {transform_id}: {e}"
        )
...
def _generate_quality_recommendations(quality_results: QualityResults) -> List[str]:
    ...
    if quality_results.overall_score < 70:
        recommendations.append(
            "Consider reviewing the ontology schema for better extraction guidance"
        )
    ...
    if warning_count > 10:
        recommendations.append(
            "High number of warnings - consider refining quality rules"
        )

    # Confidence-based recommendations
    if quality_results.metrics.avg_entity_confidence < 0.6:
        recommendations.append(
            "Low extraction confidence - consider improving source document quality"
        )

    # Completeness-based recommendations
    if quality_results.metrics.property_completeness_rate < 60:
        recommendations.append(
            "Low property completeness - review required property definitions"
        )

    # Entity distribution recommendations
    if len(quality_results.metrics.entity_type_coverage) < 2:
        recommendations.append(
            "Limited entity type diversity - verify document content matches schema"
        )

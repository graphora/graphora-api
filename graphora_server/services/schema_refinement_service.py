...
What would you like to modify first?"""
            ...
            logger.error(
                f"Failed to start refinement session for user {user_id}: {str(e)}"
            )
            raise Exception(f"Refinement session creation failed: {str(e)}") from e

    async def process_refinement_request(
        self, user_id: str, session_id: str, user_request: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Recorded user message {user_message_id} for session {session_id}"
            )
            ...
            refined_schema, changes_made, confidence, explanation = (
                await self._refine_schema_with_llm(
                    user_id=user_id,
            ...
            assistant_message_id = await self.chat_service.add_message(
                ...
                metadata={
                    ...
                    "refinement_version": conversation_context.get(
                        "refinement_count", 0
                    )
                    + 1,
                },
            ...
            logger.error(
                f"Error processing refinement request for session {session_id}: {str(e)}"
            )
        ...
        for message in reversed(messages):
            if message.get("type") == "assistant_message":
                ...
                if "refined_schema" in metadata and metadata["refined_schema"]:
                    ...
                    logger.debug(
                        f"Found refined schema from message {message.get('id', 'unknown')}"
                    )
    ...
    async def _build_conversation_context(
        ...
        if len(confidence_scores) >= 2:
            ...
            older_avg = sum(confidence_scores[:-2]) / max(
                len(confidence_scores[:-2]), 1
            )
    ...
    def _analyze_conversation_themes(
...
Refined schema:"""
    ...
    async def _fallback_refinement(
        self, current_schema: str, user_request: str
    ) -> Tuple[str, List[str], float, str]:
    ...
    async def get_current_schema(self, user_id: str, session_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Error retrieving current schema for session {session_id}: {str(e)}"
            )
            return None
...
What would you like to do next?"""
    ...
    async def _handle_refinement_error(
        self, user_id: str, session_id: str, error: Exception, user_request: str
    ) -> str:

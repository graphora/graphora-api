...
from graphora_server.services.schema_storage_service import schema_storage_service
from graphora_server.services.schema_search_service import schema_search_service
from graphora_server.utils.llm_helper import get_user_llm_credentials
from graphora_server.utils.llm_usage_tracker import track_gemini_usage
from graphora_server.utils.baml_helper import (
    create_baml_client,
    configure_baml_client_for_refinement,
)
...
class SchemaGenerationService:
    ...
    async def generate_schema(
        self, user_id: str, request: SchemaGenerationRequest
    ) -> SchemaGenerationResponse:
        ...
        try:
            ...
            generated_schema, confidence, suggestions = (
                await self._generate_schema_with_llm(
                    user_id=user_id,
                    context=context,
    ...
    async def refine_schema(
        ...
        try:
            ...
            refined_schema = self._convert_structured_schema_to_yaml(
            ...
            logger.error(
                f"Error refining schema {schema_id} for user {user_id}: {str(e)}"
            )
            raise

    def _extract_context_from_responses(
    ...
    async def _find_related_schemas(
        ...
        try:
            ...
            if "key_entities" in context:
                entities = str(context["key_entities"]).split(",")[
                    :5
                ]  # Limit to first 5
        ...
        try:
            ...
            result = b.GenerateKnowledgeGraphSchema(
                ...
                temporal_requirements=str(
                    context.get("temporal_requirements", "Not specified")
                ),
            ...
            # Convert structured result to YAML
            generated_schema = self._convert_structured_schema_to_yaml(
                result.generated_schema
            )
            ...
            logger.warning(
                f"BAML structured generation failed, falling back to template: {str(e)}"
            )
...
Generate the schema now:"""
    ...
    def _build_refinement_prompt(
        self, current_schema: str, user_feedback: str, context: Dict[str, Any]
    ) -> str:
...
Refined schema:"""
    ...
    def _parse_refinement_response(
        ...
        try:
            ...
            if "CHANGES_MADE:" in response_text:
                ...
                changes_made = [
                    c.strip() for c in changes_part.strip().split("\n") if c.strip()
                ]
    ...
    def _generate_template_based_schema(
        ...
        template = self.default_templates.get(
            domain, self.default_templates["Business/Enterprise"]
        )
  ...
  Transaction:
    ...
    def _convert_structured_schema_to_yaml(self, structured_schema) -> str:
        ...
        try:
            ...
            yaml_dict = {"version": structured_schema.version, "entities": {}}

            # Convert entities
            for entity_name, entity_data in structured_schema.entities.items():
                yaml_dict["entities"][entity_name] = {"properties": {}}
                ...
                if entity_data.properties:
                    for prop_name, prop_data in entity_data.properties.items():
                        ...
                        if (
                            hasattr(prop_data, "required")
                            and prop_data.required is not None
                        ):
                            prop_dict["required"] = prop_data.required
                        if (
                            hasattr(prop_data, "unique")
                            and prop_data.unique is not None
                        ):
                ...
                if entity_data.relationships:
                    ...
                    for rel_name, rel_data in entity_data.relationships.items():
                        rel_dict = {"target": rel_data.target}
                        ...
                        if hasattr(rel_data, "properties") and rel_data.properties:
                            ...
                            for (
                                rel_prop_name,
                                rel_prop_data,
                            ) in rel_data.properties.items():
                                rel_prop_dict = {
                                    "type": rel_prop_data.type,
                                    "description": rel_prop_data.description,
                                }
                                if (
                                    hasattr(rel_prop_data, "required")
                                    and rel_prop_data.required is not None
                                ):

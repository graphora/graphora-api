...
async def get_user_llm_credentials(user_id: str) -> Tuple[str, str]:
    ...
    if not user_config:
        raise ValueError(f"No LLM configuration found for user: {user_id}. Please configure your LLM provider first.")
    
    # Currently only supporting Gemini, but can be extended for other providers
    if user_config.provider_name != 'gemini':
        raise ValueError(f"Unsupported LLM provider: {user_config.provider_name}. Currently only Gemini is supported.")
    
    # Get the decrypted API key from the database
    # We need to query the database again to get the encrypted key and decrypt it
    response = ai_config_service.supabase.table('user_ai_configs').select(
        '''
...
def create_baml_client_registry(api_key: str, model_name: str) -> baml_py.ClientRegistry:
    """

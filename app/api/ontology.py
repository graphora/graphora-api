...
async def validate_ontology(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to store ontology"
            )
        ...
        try:
            ...
            await OntologyParser(ontology_path, user_id).build_full_text_indexes_for_user(user_id)
...
async def list_ontologies(
    ...
    try:
        ...
        result = supabase.table("ontologies").select(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error listing ontologies: {str(e)}"
        )
...
async def get_ontology_by_id(
    ...
    try:
        ...
        # Get from Supabase database only
        result = supabase.table("ontologies").select("*").eq("id", ontology_id).eq("user_id", user_id).eq("is_active", True).execute()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting ontology: {str(e)}"
        )
...
async def delete_ontology(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error deleting ontology: {str(e)}"
        )
...
async def update_ontology(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500,
                detail="Failed to update ontology"
            )
        ...
        try:
            ...
            print(f"Warning: Could not update full-text indexes for user {user_id}: {str(db_error)}")

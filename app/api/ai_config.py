...
async def create_gemini_config(
    ...
    """
    try:
        user_config = await ai_config_service.create_gemini_config(user_id, config_request)
        logger.info(f"Created Gemini configuration for user: {user_id}")
        return user_config
        
    except ValueError as e:
        logger.warning(f"Validation error creating Gemini config for {user_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail=str(e)
        )
...
async def update_gemini_config(
    ...
    """
    try:
        user_config = await ai_config_service.update_gemini_config(user_id, config_request)
        logger.info(f"Updated Gemini configuration for user: {user_id}")
        return user_config
        
    except ValueError as e:
        logger.warning(f"Validation error updating Gemini config for {user_id}: {str(e)}")
        raise HTTPException(
            status_code=400,
            detail=str(e)
        )

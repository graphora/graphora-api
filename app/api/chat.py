...
async def send_message(
    ...
    try:
        ...
        logger.info(f"Added message {message_id} to session {session_id} for user {user_id}")
        ...
        logger.error(f"Error sending message to session {session_id} for user {user_id}: {str(e)}")
...
async def get_session_history(
    ...
    try:
        ...
        if not session_data or session_data.get("total_messages", 0) == 0:
            logger.warning(f"No session data found for session {session_id} and user {user_id}")
        ...
        logger.error(f"Error retrieving session history for {session_id} and user {user_id}: {str(e)}")
...
async def refine_schema(
    ...
    try:
        ...
        if not session_id:
            ...
            logger.info(f"Started new refinement session {session_id} for user {user_id}")
        ...
        logger.info(f"Processed refinement request for session {session_id}, success: {result.get('success', False)}")
        
        return SchemaRefinementResponse(
            session_id=session_id,
            **result
        )
        ...
        raise HTTPException(status_code=500, detail="Failed to process schema refinement")
...
async def get_current_schema(
    ...
    try:
        ...
        if current_schema is None:
            raise HTTPException(
                status_code=404, 
                detail="Schema not found for the specified session"
            )
        
        logger.info(f"Retrieved current schema for session {session_id} and user {user_id}")
        ...
        logger.error(f"Error retrieving current schema for session {session_id} and user {user_id}: {str(e)}")

...
class UserDatabaseService:
    ...
    async def get_user_config(user_id: str) -> UserConfig:
        ...
        if not user_config:
            raise ValueError(f"Database configuration not found for user: {user_id}. Please configure your databases first.")
        
        return user_config
    
    @staticmethod
    async def get_database_config(user_id: str, environment: DatabaseEnvironment) -> DatabaseConfig:
        """
    ...
    @staticmethod
    async def get_graph_service(user_id: str, environment: DatabaseEnvironment) -> GraphService:
        """
        ...
        logger.info(f"Creating graph service for user {user_id} on {environment} database: {db_config.uri}")

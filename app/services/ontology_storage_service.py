...
class OntologyStorageService:
    ...
    async def _store_version_backup(self, ontology_record: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(f"Backing up ontology {ontology_record.get('id')} version {ontology_record.get('version')}")
    ...
    async def store_ontology(
        ...
        try:
            ...
            existing = self.supabase.table("ontologies").select("*").eq("id", ontology_id).eq("user_id", user_id).eq("is_active", True).execute()
            ...
            if existing.data and len(existing.data) > 0 and is_update:
                ...
                result = self.supabase.table("ontologies").update(ontology_record).eq("id", ontology_id).eq("user_id", user_id).eq("is_active", True).execute()
            ...
            if result.data and len(result.data) > 0:
                logger.info(f"Successfully stored ontology {ontology_id} for user {user_id} (update: {is_update})")
                return True
            else:
    ...
    async def get_ontology_versions(self, user_id: str, ontology_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            result = self.supabase.table("ontologies").select(
                "id, name, version, created_at, updated_at, is_active"
            ).eq("id", ontology_id).eq("user_id", user_id).eq("is_active", True).execute()

...
class QualityValidator:
        ...
        for entity_type, entity_def in self.ontology.get("entities", {}).items():
            ...
            for prop_name, prop_def in entity_def.get("properties", {}).items():
                ...
                if quality_config:
                    ...
                    logger.debug(
                        f"Added {len(rules)} quality rules for {entity_type}.{prop_name}"
                    )

                # Add required property rule if property is required
                if prop_def.get("required", False):
                    required_rule = self._create_required_property_rule(
            ...
            if entity_quality:
                rules = self.parser.parse_entity_quality_rules(
                    entity_type, entity_quality
                )
    ...
    def _create_required_property_rule(
        self, entity_type: str, prop_name: str
    ) -> QualityRule:
    ...
    async def validate_extraction(
        self, knowledge_graph: DocumentKnowledgeGraph, transform_id: str
    ) -> QualityResults:
        ...
        try:
            ...
            relationship_violations = await self._validate_relationships(
            ...
            all_violations = (
                entity_violations + relationship_violations + global_violations
            )
            ...
            violations_by_entity_type = self._group_violations_by_entity_type(
                all_violations
            )
        ...
        for entity in entities:
            ...
            for prop_name, prop_value in entity.properties.items():
                ...
                for rule in prop_rules:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Rule {rule.rule_id} failed on {entity_type}.{prop_name}: {e}"
                        )
            ...
            for rule in entity_level_rules:
                ...
                try:
                    ...
                    logger.error(
                        f"Entity-level rule {rule.rule_id} failed on {entity_type}: {e}"
                    )
        ...
        for relationship in relationships:
            ...
            for rule in rules:
                ...
                try:
                    ...
                    logger.error(
                        f"Relationship rule {rule.rule_id} failed on {rel_type}: {e}"
                    )
    ...
    def _calculate_metrics(
        self, kg: DocumentKnowledgeGraph, violations: List[QualityViolation]
    ) -> QualityMetrics:
        ...
        entities_with_violations = len(
            set(v.entity_id for v in violations if v.entity_id)
        )
        relationships_with_violations = len(
            set(
        ...
        # Calculate rates
        entity_violation_rate = (
            entities_with_violations / max(total_entities, 1)
        ) * 100
        relationship_violation_rate = (
            relationships_with_violations / max(total_relationships, 1)
        ) * 100
        overall_violation_rate = (
            len(violations) / max(total_entities + total_relationships, 1)
        ) * 100
        ...
        if required_properties > 0:
            property_completeness_rate = (
                filled_required_properties / required_properties
            ) * 100
            logger.info(
            ...
            property_completeness_rate = (
                total_filled_properties / total_expected_properties
            ) * 100
            logger.info(
            ...
            property_completeness_rate = (
                100.0  # No properties defined, consider complete
            )
            logger.info(
                "No properties defined in ontology, setting completeness to 100%"
            )
    ...
    def _calculate_overall_score(
        self, metrics: QualityMetrics, violations: List[QualityViolation]
    ) -> float:
        ...
        error_penalty = (
            len([v for v in violations if v.severity == QualitySeverity.ERROR]) * 10
        )
        warning_penalty = (
            len([v for v in violations if v.severity == QualitySeverity.WARNING]) * 5
        )
        info_penalty = (
            len([v for v in violations if v.severity == QualitySeverity.INFO]) * 1
        )
        ...
        completeness_bonus = (
            metrics.property_completeness_rate - 50
        ) * 0.2  # Bonus/penalty for completeness

        # Factor in confidence scores
        confidence_bonus = (
            metrics.avg_entity_confidence - 0.5
        ) * 20  # Bonus/penalty for confidence
    ...
    def _requires_human_review(
        self, violations: List[QualityViolation], score: float
    ) -> bool:
    ...
    def _group_violations_by_entity_type(
        self, violations: List[QualityViolation]
    ) -> Dict[str, int]:
    ...
    def _build_entity_quality_summary(
        self, violations: List[QualityViolation]

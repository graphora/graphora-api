...
class QualityService:
    ...
    async def get_quality_results(
        ...
        try:
            ...
            logger.error(f"Failed to retrieve quality results for transform {transform_id}: {e}")
            return None
    ...
    async def approve_quality_results(
        ...
        try:
            ...
            logger.info(f"Quality results approved for transform {transform_id} by user {user_id}")
            
        except Exception as e:
            logger.error(f"Failed to approve quality results for transform {transform_id}: {e}")
            raise
    ...
    async def reject_quality_results(
        ...
        try:
            ...
            logger.info(f"Quality results rejected for transform {transform_id} by user {user_id}: {rejection_reason}")
            
        except Exception as e:
            logger.error(f"Failed to reject quality results for transform {transform_id}: {e}")
            raise
        ...
        try:
            ...
            for result in results:
                summary = {
                    ...
                    'status': 'approved' if result.get('approved') else ('rejected' if result.get('rejected') else 'pending')
                }
    ...
    async def delete_quality_results(
        ...
        try:
            ...
            if deleted_count > 0:
                ...
                logger.warning(f"No quality results found to delete for transform {transform_id}")
                return False
                
        except Exception as e:
            logger.error(f"Failed to delete quality results for transform {transform_id}: {e}")
            return False
    ...
    def calculate_auto_approval_eligibility(
        ...
        if quality_results.overall_score < threshold:
            eligible = False
            reasons.append(f"Score {quality_results.overall_score:.1f} below threshold {threshold}")
        
        # Check error count
        if auto_approval_config.get('no_errors_required', True):
            error_count = quality_results.violations_by_severity.get(QualitySeverity.ERROR, 0)
            if error_count > 0:
                eligible = False
        ...
        warning_count = quality_results.violations_by_severity.get(QualitySeverity.WARNING, 0)

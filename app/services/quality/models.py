...
class QualityViolation(BaseModel):
    ...
    entity_type: Optional[str] = Field(
        None, description="Type of entity where violation occurred"
    )
    entity_id: Optional[str] = Field(None, description="ID of the specific entity")
    property_name: Optional[str] = Field(
        None, description="Property name where violation occurred"
    )
    relationship_type: Optional[str] = Field(
        None, description="Type of relationship if applicable"
    )
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence in this violation detection",
    )
    ...
    context: Dict[str, Any] = Field(
        default_factory=dict, description="Additional context for the violation"
    )
...
class QualityMetrics(BaseModel):
    ...
    total_properties: int = Field(
        description="Total number of properties across all entities"
    )

    # Violation statistics
    entities_with_violations: int = Field(
        description="Number of entities that have violations"
    )
    relationships_with_violations: int = Field(
        description="Number of relationships that have violations"
    )
    total_violations: int = Field(description="Total number of violations found")

    # Rates and percentages
    entity_violation_rate: float = Field(
        description="Percentage of entities with violations"
    )
    relationship_violation_rate: float = Field(
        description="Percentage of relationships with violations"
    )
    overall_violation_rate: float = Field(description="Overall violation rate")

    # Confidence scores
    avg_entity_confidence: float = Field(
        description="Average confidence score for entity extraction"
    )
    avg_relationship_confidence: float = Field(
        description="Average confidence score for relationship extraction"
    )
    confidence_scores_by_type: Dict[str, float] = Field(
        default_factory=dict, description="Average confidence by entity type"
    )

    # Completeness metrics
    property_completeness_rate: float = Field(
        description="Percentage of required properties that were filled"
    )
...
class QualityResults(BaseModel):
    ...
    # Summary breakdowns
    violations_by_type: Dict[QualityRuleType, int] = Field(
        default_factory=dict, description="Violation count by rule type"
    )
    violations_by_severity: Dict[QualitySeverity, int] = Field(
        default_factory=dict, description="Violation count by severity"
    )
    violations_by_entity_type: Dict[str, int] = Field(
        default_factory=dict, description="Violation count by entity type"
    )
    ...
    validation_duration_ms: int = Field(
        description="How long validation took in milliseconds"
    )
    rules_applied: int = Field(description="Number of quality rules that were applied")

    # Configuration used
    validation_config: Dict[str, Any] = Field(
        default_factory=dict, description="Configuration used for validation"
    )
...
class ValidationResult(BaseModel):
    ...
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Confidence in the validation result"
    )
    violations: List[QualityViolation] = Field(
        default_factory=list, description="Any violations found"
    )
...
class QualityRuleConfig(BaseModel):
    ...
    severity: QualitySeverity = Field(
        default=QualitySeverity.WARNING, description="Severity of violations"
    )
    ...
    examples: List[str] = Field(
        default_factory=list, description="Examples of valid/invalid values"
    )

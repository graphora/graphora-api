...
from app.services.marker.client import MarkerAPIClient, ConversionError, MarkerResponse
...
class ConversionStorage:
    ...
    async def store_conversion_result(
        self, transform_id: str, result: MarkerResponse, original_path: Path
    ) -> ConversionResult:
        ...
        md_path = markdown_dir / "markdown.md"
...
async def handle_conversion_error(
    error: Exception, transform_id: str, retry_count: int
) -> None:
    ...
    logger.error(f"Conversion error for transform {transform_id}", extra=error_context)
...
async def convert_pdf_to_markdown(
    file_path: Path, transform_id: str
    ...
    try:
        ...
        return await storage.store_conversion_result(
            transform_id=transform_id, result=result, original_path=file_path
        )
...
@task(name="marker-api-health-check", retries=2, tags=["monitoring"])
async def check_marker_api_health() -> None:
    ...
    try:
        client = MarkerAPIClient(
            host=settings.MARKER_API_HOST, timeout=10  # Short timeout for health check
        )

        status = await client.check_health()
        logger.info("Marker API health status", extra=status.model_dump())
        ...
        traceback.print_exc()
        logger.error(
            f"Marker API health check failed: {str(e)}", extra={"alert": "api_down"}
        )

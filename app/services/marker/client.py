...
from app.services.marker.models import MarkerResponse, ConversionMetadata, HealthStatus
...
class MarkerAPIClient:
    ...
    async def convert_to_markdown(
        self, file_path: Path, use_llm: bool = False, paginate: bool = True
    ) -> MarkerResponse:
        """
        ...
        try:
            async with self._init_session() as session:
                ...
                for attempt in range(self.max_retries):
                    try:
                        logger.debug(
                            f"Starting PDF conversion attempt {attempt + 1}/{self.max_retries}"
                        )
                        logger.info(
                            f"Starting PDF conversion attempt {attempt + 1}/{self.max_retries}"
                        )
                        async with session.post(
                            f"{self.base_url}/marker/upload", data=data
                        ) as response:
                        ...
                        logger.warning(
                            f"PDF conversion attempt {attempt + 1} failed: {str(e)}"
                        )
                ...
                return MarkerResponse(
                    ...
                    conversion_metadata=ConversionMetadata(
                        ...
                        settings={"use_llm": use_llm, "paginate": paginate},
                    ),
                )

...
async def load_ontology(ontology_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if ontology_path.exists():
            try:
                with open(ontology_path, 'r') as f:
                    ...
                    logger.info(f"Successfully loaded ontology '{ontology_id}' from local file")
                    return ontology_data
            except (IOError, yaml.YAMLError) as e:
                logger.warning(f"Failed to read local ontology file {ontology_path}: {e}")
        ...
        if user_id:
            ...
            if db_content:
                ...
                logger.warning(f"Ontology '{ontology_id}' not found in database for user {user_id}")
    ...
    try:
        ...
        if result.data and len(result.data) > 0:
            ...
            try:
                ...
                logger.warning(f"Ontology '{ontology_id}' from database contains invalid YAML: {e}")
                return None
        else:

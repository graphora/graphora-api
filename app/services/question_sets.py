...
def get_question_sets_for_domain(domain: str = None, include_optional: bool = True) -> List[QuestionSet]:
...
def validate_user_responses(responses: List[dict]) -> List[str]:
    ...
    for response in responses:
        ...
        if question.required and (not value or (isinstance(value, str) and not value.strip())):
        ...
        if question.validation and value:
            ...
            if isinstance(value, str):
                if validation.get('min_length') and len(value) < validation['min_length']:
                    errors.append(f"Answer for '{question.prompt}' is too short (minimum {validation['min_length']} characters)")
                
                if validation.get('max_length') and len(value) > validation['max_length']:
                    errors.append(f"Answer for '{question.prompt}' is too long (maximum {validation['max_length']} characters)")
                
                if validation.get('pattern'):
                    import re
                    if not re.match(validation['pattern'], value):
                        errors.append(f"Answer for '{question.prompt}' does not match required format")
        
        # Validate select options
        if question.type in [QuestionType.SELECT, QuestionType.MULTISELECT] and question.options:
            if question.type == QuestionType.SELECT:
                ...
                if isinstance(value, list):
                    ...
                    if invalid_options:
                        errors.append(f"Invalid options selected for '{question.prompt}': {invalid_options}")
                else:
                    errors.append(f"Multiple selection expected for '{question.prompt}'")

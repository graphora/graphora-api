...
class ChatSessionService:
    ...
    async def start_session(
        ...
        try:
            ...
            logger.info(f"Started chat session {session_id} for user {user_id} with context {context_type.value}")
            return session_id
    ...
    async def add_message(
        ...
        try:
            ...
            logger.debug(f"Added {message_type.value} message {message_id} to session {session_id}")
            return message_id
    ...
    async def get_session_history(
        ...
        try:
            ...
            query = self.audit_service.supabase.table("audit_trail").select("*").eq(
                "user_id", user_id
            ).in_(
                "operation_type", [
            ...
            ).limit(limit + 1)  # +1 to account for session start entry
            ...
            for entry in result.data:
                ...
                if entry["operation_type"] == OperationType.CHAT_SESSION_STARTED.value:
                    ...
                    message_metadata = {
                        ...
                        if k not in ["session_id", "message_id", "message_type", "content", "timestamp"]
                    }
    ...
    async def get_user_sessions(
        ...
        try:
            ...
            query = self.audit_service.supabase.table("audit_trail").select("*").eq(
                ...
                "operation_type", OperationType.CHAT_SESSION_STARTED.value
            ).order(
            ...
            for entry in result.data:
                ...
                try:
                    ...
                    logger.warning(f"Failed to get message count for session {session_id}: {count_error}")
                    message_count = 0
    ...
    async def _get_session_message_count(self, user_id: str, session_id: str) -> int:
        ...
        try:
            result = self.audit_service.supabase.table("audit_trail").select(
                "operation_id", count="exact"
            ).eq(
                "user_id", user_id
            ).in_(
                "operation_type", [

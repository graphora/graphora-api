...
class FeedbackService:
    ...
    async def store_quality_feedback(
        ...
        try:
            ...
            result = self.client.table("quality_feedback").insert(feedback_data).execute()
            
            if result.data:
                logger.info(f"Quality feedback stored successfully for user {user_id}, transform {transform_id}")
                return True
            else:
    ...
    async def get_user_feedback(
        ...
        try:
            result = self.client.table("quality_feedback")\
                .select("*")\
                .eq("user_id", user_id)\

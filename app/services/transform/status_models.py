...
class ErrorSummary(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ErrorSummary":
        ...
        if isinstance(data["error_timestamp"], str):
            data["error_timestamp"] = datetime.fromisoformat(data["error_timestamp"])
        return cls(**data)
...
class StageProgress(BaseModel):
    ...
    def update_progress(
        ...
        self.percentage_complete = (
            (items_processed / items_total) * 100 if items_total > 0 else 0.0
        )
        if metrics:
...
class DetailedTransformStatus(BaseModel):
    ...
    def percentage_complete(self) -> float:
        ...
        total_percentage = sum(
            stage.percentage_complete for stage in self.stages_progress.values()
        )
    ...
    def update_stage_progress(
        ...
        if stage in self.stages_progress:
            self.stages_progress[stage].update_progress(
                items_processed, items_total, metrics
            )
    ...
    def fail_stage(self, stage: TransformationStage, error: ErrorSummary):
    ...
    def estimate_completion_time(
        self, historical_timings: Dict[TransformationStage, List[float]]
    ):
        ...
        try:
            ...
            for stage, progress in self.stages_progress.items():
                if not progress.is_complete:
                    ...
                    if progress.percentage_complete > 0:
                        remaining = (
                            avg_time * (100 - progress.percentage_complete) / 100
                        )
                    else:
                        remaining = avg_time
            ...
            if remaining_ms > 0:
                self.estimated_completion_time = datetime.now().fromtimestamp(
                    datetime.now().timestamp() + (remaining_ms / 1000)
                )

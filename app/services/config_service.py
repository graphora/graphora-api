...
class ConfigService:
    ...
    async def update_user_config(self, user_id: str, config_request: ConfigRequest) -> UserConfig:
        ...
        try:
            ...
            config_response = self.supabase.table('configs').update({
                'updated_at': datetime.now(timezone.utc).isoformat()
            }).eq('id', existing_config.id).execute()
    ...
    async def delete_user_config(self, user_id: str) -> bool:
        ...
        try:
            ...
            config_response = self.supabase.table('configs').delete().eq('id', existing_config.id).execute()

...
async def get_patients(
    ...
    try:
        ...
        if limit > 1000:
            raise HTTPException(
                status_code=400,
                detail="Maximum limit is 1000 patients"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving patients data: {str(e)}"
        )
...
async def get_patient_journey(
    ...
    """
    try:
        patient_journey = await healthcare_service.get_patient_journey(patient_id=patient_id)

...
class AIModel(BaseModel):
    ...
    version: Optional[str] = Field(None, description="Model version (e.g., '001', 'latest')")
...
class AIProviderConfig(BaseModel):
    ...
    config_data: Dict[str, Any] = Field(default_factory=dict, description="Additional provider-specific settings")
...
class UserAIConfig(BaseModel):
    ...
    active_provider_config_id: str = Field(..., description="Active AI provider configuration")
...
class GeminiConfigRequest(BaseModel):
    ...
    default_model_name: str = Field(..., description="Default model name (e.g., 'gemini-1.5-pro')")
...
class UserAIConfigDisplay(BaseModel):
    ...
    default_model_display_name: str = Field(..., description="Human-readable model name")
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None
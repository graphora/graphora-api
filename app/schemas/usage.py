...
class ModelPricingSchema(BaseModel):
    ...
    input_price_per_1k_tokens: Decimal = Field(
        ..., description="Input token price per 1K"
    )
    output_price_per_1k_tokens: Decimal = Field(
        ..., description="Output token price per 1K"
    )

    # Additional metadata
    model_context_window: Optional[int] = Field(
        None, description="Model context window size"
    )
...
class DocumentUsage(BaseModel):
    ...
    is_reprocessing: bool = Field(
        default=False, description="Whether this is a reprocessing"
    )
    reprocessing_reason: Optional[str] = Field(
        None, description="Reason for reprocessing"
    )
    ...
    billable_processing_units: int = Field(
        default=1, description="Processing units for billing"
    )
...
class LLMUsage(BaseModel):
    ...
    transform_id: Optional[str] = Field(
        None, description="Associated transformation ID"
    )
    document_usage_id: Optional[str] = Field(
        None, description="Associated document usage ID"
    )
    ...
    cost_per_1k_input_tokens: Optional[Decimal] = Field(
        None, description="Input token cost rate"
    )
    cost_per_1k_output_tokens: Optional[Decimal] = Field(
        None, description="Output token cost rate"
    )
    ...
    response_timestamp: Optional[datetime] = Field(
        None, description="When response received"
    )
...
class UsageAggregate(BaseModel):
    ...
    avg_pages_per_document: Optional[Decimal] = Field(
        None, description="Average pages per document"
    )
    ...
    total_estimated_cost_usd: Optional[Decimal] = Field(
        None, description="Total estimated cost"
    )

    # Model breakdown (JSON for flexibility)
    model_usage_breakdown: Optional[Dict[str, Any]] = Field(
        None, description="Usage by model"
    )

    # Performance metrics
    avg_tokens_per_page: Optional[Decimal] = Field(
        None, description="Average tokens per page"
    )
    avg_processing_time_per_page_ms: Optional[int] = Field(
        None, description="Average processing time per page"
    )
...
class PricingTier(BaseModel):
    ...
    # Limits
    monthly_document_limit: Optional[int] = Field(
        None, description="Monthly document limit"
    )
    ...
    monthly_cost_limit_usd: Optional[Decimal] = Field(
        None, description="Monthly cost limit"
    )
    ...
    # Pricing
    base_price_usd: Decimal = Field(
        default=Decimal("0"), description="Base monthly price"
    )
    price_per_page_usd: Optional[Decimal] = Field(None, description="Price per page")
    price_per_1k_tokens_usd: Optional[Decimal] = Field(
        None, description="Price per 1K tokens"
    )
...
class UsageReport(BaseModel):
    ...
    # Performance metrics
    avg_processing_time_ms: Optional[int] = Field(
...
class LimitCheckResult(BaseModel):
    ...
    # Remaining
    remaining_documents: Optional[int] = Field(
        None, description="Remaining document capacity"
    )
    remaining_pages: Optional[int] = Field(None, description="Remaining page capacity")
    remaining_tokens: Optional[int] = Field(
        None, description="Remaining token capacity"
    )
    remaining_cost_usd: Optional[Decimal] = Field(
        None, description="Remaining cost capacity"
    )

    # Warnings
    warnings: List[str] = Field(default=[], description="Usage warnings")
    upgrade_recommended: bool = Field(
        default=False, description="Whether upgrade is recommended"
    )
